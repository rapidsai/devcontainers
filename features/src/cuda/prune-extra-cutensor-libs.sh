...
for libcutensor_lib in libcutensor1 libcutensor2; do
    if ! dpkg -s "$libcutensor_lib" >/dev/null 2>&1; then
        continue;
    fi
    libcutensor_ver="$(dpkg -s "$libcutensor_lib" | grep '^Version:' | cut -d' ' -f2 | cut -d'-' -f1 | cut -d'.' -f4 --complement)";
