...
try {
    ...
    buildctl build `
        ...
        --opt build-arg:MSVC_VER="$ENV:MSVC_VER" `
        --opt build-arg:MSVC_COMPILER_VER="$ENV:MSVC_COMPILER_VER" `
        --opt build-arg:CUDA_VER="$ENV:CUDA_VER" `
        --opt build-arg:ROOT_IMAGE="$ENV:ROOT_IMAGE" `
        .\image
}
catch {
    Pop-Location
